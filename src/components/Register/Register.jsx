...
function Register(props) {
    ...
    const handleChange = (event) => {
        ...
        if (EmailValidator.validate(value) === false) {
            console.log(inputName.validity)
            inputEmail.setCustomValidity('Поле Email должно содержать действительный e-mail адрес.')
    ...
    return (
        <main className="register">
            ...
            <form className="sign__form" onSubmit={isValid && handleSubmit} onChange={handleChange} values={values} errors={errors}>
                    ...
                    <span id="input-password-error" name='password' className={`error ${!isValid && 'error_type_visible'}`}>{errors ? errors['input-password-error'] : ''}</span>
                    <p className={`info-tool-tip ${(props.isSuccess || props.isFail) && 'info-tool-tip_type_visible'}`}>{props.toolText}</p>
                    <button className="sign__button-submit" type="submit" value='save'>Зарегистрироваться</button>
                </form>
